...
class Mover(object):
    ...
    def callback(self,data):
            print data.value, self.x,self.y
            if (data.pose.position.x == self.x)&(self.y == data.pose.position.y):
                self.i+=1
                self.y = self.i/1000.0#(self.i%1501)*0.001 -0.75
                self.x = 0.000#(np.floor(self.i/1501))*0.001 -0.75
    ...
    def __init__(self):
        ...
        self.i = 0
        self.filename = "/home/tom/Desktop/data/line.csv"
        ...
        try:
            self.i = 0      
            self.y = 0.0#(self.i%151)*0.01 -0.75
            self.x = 0.000#(np.floor(self.i/151))*0.01 -0.75
            while self.i < 28000:
